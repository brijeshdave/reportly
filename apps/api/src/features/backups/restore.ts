...
async function restoreDatabaseDump(dump: Buffer): Promise<void> {
  ...
  const { code, stderr } = await runCapture(
    cmd!,
    [...prefix, "--clean", "--if-exists", "--no-owner", "--no-privileges", "-d", env.DATABASE_URL],
    dump,
  );
  if (code !== 0) {
    logger.error({ feature: "backups", stderr: stderr.slice(0, 2000) }, "Database restore failed");
    throw new AppError(
      500,
...
async function restoreFilesArchive(archive: Buffer): Promise<void> {
  ...
  if (code !== 0) {
    logger.error({ feature: "backups", stderr: stderr.slice(0, 2000) }, "Files restore failed");
    throw new AppError(
      500,
      ERROR_CODES.INTERNAL_ERROR,
